...
export const LikeList = ({ showList, user, setShowList, logedUser,setLogedUser }) => {
                ...
                <div className="w-full py-1.5 flex items-center ">
                  <div className="flex items-center justify-center">
                    <img
                      ...
                      onClick={() => {navigate(`/profile/${item.username}`, { state: { user:item } });setShowList('')}}
                      alt=""
                      className="w-11 h-11 rounded-full object-cover cursor-pointer"
                    />
                  </div>
                  <div className="flex-col flex text-[14px] ml-4  justify-center">
                    <span className="font-medium leading-4 cursor-pointer"
                    onClick={() => {navigate(`/profile/${item.username}`, { state: { user:item } });setShowList('')}}>
                      {item?.username}
                    </span>
                  ...
                  {item._id != logedUser._id && (
                    <div className="flex-1 justify-end flex">
                      <button
                      onClick={()=>{getFollower(item?._id);handleclick(item?._id)}}
